								 ...
								 int nrhs, const mxArray *prhs[])
{
	...
	/* Send the string */
    // Send "0" at the end to indicate message end
    cdata = (unsigned char *)mxGetPr(prhs[1]);
    
    unsigned char *cdata_aug = new unsigned char[len+1];
    for (int k=0;k<len;k++) cdata_aug[k] = cdata[k];
    cdata_aug[len]=0;
    
	while(cnt < len+1) {
		ret[0] = send(sock,(const char *)cdata_aug+cnt,(len+1)-cnt,0);
		if(ret[0] == -1) {
			perror("send");
			return;
		}
