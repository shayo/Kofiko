...
/*    
    // Test whether socket is ready for writing 
	FD_ZERO(&readfds);
	FD_ZERO(&writefds);
	FD_ZERO(&exceptfds);
	...
	tv.tv_sec = 0;
	tv.tv_usec = 0;
	select(sock+1,&readfds,&writefds,&exceptfds,&tv); // returns immideatly

	int Tmp1 = FD_ISSET(sock,&writefds);
	int Tmp2 = FD_ISSET(sock,&exceptfds);
    mexPrintf("Write = %d\n",Tmp1);
    mexPrintf("Exp = %d\n",Tmp2);
    */
