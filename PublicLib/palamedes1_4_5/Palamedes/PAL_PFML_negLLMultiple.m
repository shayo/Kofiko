...
if (~isempty(lapseLimits) && (min(params(:,4)) < lapseLimits(1) || max(params(:,4)) > lapseLimits(2))) || (~isempty(guessLimits) && (min(params(:,3)) < guessLimits(1) || max(params(:,3)) > guessLimits(2)))
    ...
    for Cond = 1:size(StimLevels,1)
        ...
        negLLcond(Cond) = PAL_PFML_negLL([params(Cond,1) params(Cond,2) params(Cond,3) params(Cond,4)], [], [1 1 1 1], SL, NP, OoN,PF,'lapseLimits', lapseLimits,'guessLimits', guessLimits,'lapseFit',lapseFit,'gammaEQlambda',gammaEQlambda);
    end
    negLL = sum(negLLcond);
end