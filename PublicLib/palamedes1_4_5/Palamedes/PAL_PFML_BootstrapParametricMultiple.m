...
%   ftrouble if this function was used with data-arrays that would have 
...
if ~isempty(lapseLimits) && (PAL_whatIs(ArgsL) == 1 || PAL_whatIs(ArgsL) == 4)
    message = ['''lapseLimits'' not compatible with chosen modeling of lapse rates: ''lapseLimits'' ignored'];
    warning(message);
    lapseLimits = [];
end
if ~isempty(guessLimits) && (PAL_whatIs(ArgsG) == 1 || PAL_whatIs(ArgsG) == 4)
