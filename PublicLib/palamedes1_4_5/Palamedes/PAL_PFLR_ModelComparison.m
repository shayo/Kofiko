...
if ~isempty(lapseLimits) && (PAL_whatIs(MC.argsLlesser) == 1 || PAL_whatIs(MC.argsLlesser) == 4 || PAL_whatIs(MC.argsLfuller) == 1 || PAL_whatIs(MC.argsLfuller) == 4)
    message = ['''lapseLimits'' not compatible with chosen modeling of lapse rates: ''lapseLimits'' ignored'];
    warning(message);
    lapseLimits = [];
end
if ~isempty(guessLimits) && (PAL_whatIs(MC.argsGlesser) == 1 || PAL_whatIs(MC.argsGlesser) == 4 || PAL_whatIs(MC.argsGfuller) == 1 || PAL_whatIs(MC.argsGfuller) == 4)
    message = ['''guessLimits'' not compatible with chosen modeling of guess rates: ''guessLimits'' ignored'];
    warning(message);
    guessLimits = [];
end
