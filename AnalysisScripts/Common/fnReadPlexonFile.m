function strctPlexon = fnReadPlexonFile(strInputFile, acAnalogInputsRequired, acAnalogStructNames, bReadSpikes)
%
...
if g_bVERBOSE
    ...
    fprintf('Using version %d at Freq %d \n',strctPlexon.m_strctInfo.m_fVersion, strctPlexon.m_strctInfo.m_fFreq);
end;
...
try
    ...
    strctPlexon.m_strctStrobeWord.m_afTimestamp = [];
end
...
[a2iSpikeCount, wfcounts, evcounts] = plx_info(strInputFile, 1);
aiActiveSpikeChannels = find(sum(a2iSpikeCount,1) > 0)-1;


if ~isempty(acAnalogInputsRequired)
    if g_bVERBOSE
        fprintf('Reading analog signals...\n');
    end;
    for k=1:length(acAnalogInputsRequired)
        iIndex = find(ismember(strctPlexon.m_acAnalogLineNames,acAnalogInputsRequired{k}));
        if isempty(iIndex);
            fnWorkerLog('Warning, could not find analog signal %s',acAnalogInputsRequired{k});
        else
            fnWorkerLog('Reading analog channel %s',acAnalogInputsRequired{k});
            strVarName = ['m_strct',acAnalogStructNames{k}];
            strVarName(strVarName == ' ') = '_';
            strctPlexon = setfield(strctPlexon, strVarName, fnReadAnalog(strInputFile, iIndex-1));
        end;
    end;
end;
...
if bReadSpikes
    try
        for iUnitIter=1:4
            strctPlexon.m_astrctUnits(iUnitIter) = fnReadSpikes(strInputFile, 1,iUnitIter);
            fnWorkerLog('Timestamps for channel 1, unit %d was found',iUnitIter);
        end;
    catch
    end;
...
function strctSpikes = fnReadSpikes(strInputFile, iChannel, iUnit)
...
function strctAnalog=fnReadAnalog(strInputFile, iIndex)
[strctAnalog.m_fFreq, ...
