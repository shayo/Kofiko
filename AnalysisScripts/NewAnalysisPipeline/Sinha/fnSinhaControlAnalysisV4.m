function strctUnit = fnSinhaControlAnalysisV4(strctUnit, strctKofiko, strctInterval,strctConfig,aiTrialIndices)
...
strctHairComparison.m_a2bCondPValue = nans(iNumConditions,iNumConditions);
strctHairComparison.m_a2bCondPValueBaseSub = nans(iNumConditions,iNumConditions);
strctHairComparison.m_afAvgFiringCondition = nans(1,iNumConditions);
strctHairComparison.m_afAvgFiringConditionBaseSub = nans(1,iNumConditions);
for iCond1=1:iNumConditions
    ...
    for iCond2=1:iNumConditions
        ...
        fNaNPerc1 = sum(isnan(afCond1)) / length(afCond1);
        fNaNPerc2 = sum(isnan(afCond2)) / length(afCond2);
        if fNaNPerc1 > 0.3 || fNaNPerc2 > 0.3
            continue;
        end;
...
strctSinhaFeatureTuning.m_a2fTuning = nans(iNumConditions,11);
strctSinhaFeatureTuning.m_a2fTuningBaseLineSub = nans(iNumConditions,11);
strctSinhaFeatureTuning.m_afSigTuning = nans(1,iNumConditions);
strctSinhaFeatureTuning.m_afSigTuningBaseSub = nans(1,iNumConditions);

afAvgFirintRate_Stimulus70_270=nanmean(strctUnit.m_a2fAvgFirintRate_Stimulus(:,271:471),2);
afAvgFirintRate_Stimulus0_50=nanmean(strctUnit.m_a2fAvgFirintRate_Stimulus(:,201:251),2);
afAvgFirintRate_StimulusBaselineSub = afAvgFirintRate_Stimulus70_270-afAvgFirintRate_Stimulus0_50;

for iCondIter=1:iNumConditions
    % Build the curve, and test the extreme 
    
    if iCondIter ~= 4
        strctSinhaFeatureTuning.m_a2fTuning(iCondIter,:) = afAvgFirintRate_Stimulus70_270( acCondInd{iCondIter});
        strctSinhaFeatureTuning.m_a2fTuningBaseLineSub(iCondIter,:) = afAvgFirintRate_StimulusBaselineSub( acCondInd{iCondIter});
        afRange = -5:5;
        fNaNperc = sum(isnan(strctSinhaFeatureTuning.m_a2fTuning(iCondIter,:)))/length(afRange);
        if fNaNperc < 0.3
        ...
        strctSinhaFeatureTuning.m_a2fTuning(iCondIter,3:11) = afAvgFirintRate_Stimulus70_270( acCondInd{iCondIter});
        strctSinhaFeatureTuning.m_a2fTuningBaseLineSub(iCondIter,3:11) = afAvgFirintRate_StimulusBaselineSub( acCondInd{iCondIter});
        afRange=-3:5;
        fNaNperc = sum(isnan(strctSinhaFeatureTuning.m_a2fTuning(iCondIter,:)))/length(afRange);
        if fNaNperc < 0.3
...
fNaNPerc = sum(isnan(strctSinhaFeatureTuning.m_a2fTuning(:))) / length(strctSinhaFeatureTuning.m_a2fTuning(:));
if fNaNPerc > 0.3
    strctSinhaFeatureTuning = [];
    return;
end;
...
fNaNPerc = sum(isnan(strctUnit.m_afAvgFirintRate_Stimulus(aiStimuliIndex))) / length(aiStimuliIndex);
if fNaNPerc > 0.3
    strctPartsANOVA = [];
    return;
end
...
for iCondIter=1:iNumConditions
    % Build the curve, and test the extreme 
    if iCondIter ~= 4
        strctCartoonFeatureTuning.m_a2fTuning(iCondIter,:) = afAvgFirintRate_Stimulus70_270( acCondInd{iCondIter});
        strctCartoonFeatureTuning.m_a2fTuningBaseLineSub(iCondIter,:) = afAvgFirintRate_StimulusBaselineSub( acCondInd{iCondIter});
        afRange = -5:5;
       ...
       strctCartoonFeatureTuning.m_afSigTuningBaseSub(iCondIter) = fnSafeRobustFit(afRange,strctCartoonFeatureTuning.m_a2fTuningBaseLineSub(iCondIter,:));
        ...
        strctCartoonFeatureTuning.m_a2fTuning(iCondIter,3:11) = afAvgFirintRate_Stimulus70_270( acCondInd{iCondIter});
        strctCartoonFeatureTuning.m_a2fTuningBaseLineSub(iCondIter,3:11) = afAvgFirintRate_StimulusBaselineSub( acCondInd{iCondIter});
        afRange=-4:4;
...
fNaNPerc = sum(isnan(strctCartoonFeatureTuning.m_a2fTuning(:))) / length(strctCartoonFeatureTuning.m_a2fTuning(:));
if fNaNPerc > 0.3
    strctCartoonFeatureTuning = [];
    return;
end;
...
for iCondIter1=1:iNumConditions
    for iCondIter2=1:iNumConditions
        afCond1 =  strctUnit.m_afAvgFirintRate_Stimulus(acCondInd{iCondIter1});
        afCond2 = strctUnit.m_afAvgFirintRate_Stimulus(acCondInd{iCondIter2});
        fNanPerc1 = sum(isnan(afCond1)) / length(afCond1);
        fNanPerc2 = sum(isnan(afCond2)) / length(afCond2);
        if fNanPerc1 > 0.7 && fNanPerc2 > 0.7
            a2fPValue(iCondIter1,iCondIter2) = ttest(afCond1, afCond2);
