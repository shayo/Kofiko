function acUnitsStat = fnCollectForceChoiceStats(strRawFolder,strSession,strctKofiko, strctSync, strctConfig, strctInterval, strOutputFolder,strTrainFile1,strTrainFile2)
...
for iListIter=1:iNumUniqueLists
   ...
   strctDesign = strctKofiko.g_astrctAllParadigms{iParadigmIndex}.ExperimentDesigns.Buffer{1+aiListIndicesInArray(1)};
    ...
    if ~isempty(aiRelevantTrialsInd) && length(aiRelevantTrialsInd) > iMinimumTrialsToAnalyze
        fnCollectForceChoiceSpecificDesignAux(strRawFolder,strSession,strctKofiko, strctSync, strctConfig, strctInterval, ...
            strctDesign, aiRelevantTrialsInd, iParadigmIndex,strOutputFolder,strTrainFile1,strTrainFile2);
    end
end 


function fnCollectForceChoiceSpecificDesignAux(strRawFolder,strSession,...
            strctKofiko, strctSync, strctConfig, strctInterval, strctDesign, aiTrialInd, iParadigmIndex,strOutputFolder,strTrainFile1,strTrainFile2)
...
for k=1:iNumTrials
    ...
    afCueOnsetTS_Kofiko(k) = acTrials{k}.m_strctTrialOutcome.m_afCueOnset_TS_Kofiko;
...
% 
% 
%     if ~isempty(strTrainFile1)
%         afCueOnsetTS_PLX = fnTimeZoneChange(afCueOnsetTS_Kofiko,strctSync,'Kofiko','Plexon');
%         afTimeRange = -500:2500;
%         a2fSampleTime_PLX = zeros(iNumTrials, length(afTimeRange));
%         for k=1:iNumTrials
%             a2fSampleTime_PLX(k,:) = afCueOnsetTS_PLX(k) + afTimeRange/1e3;
%         end
%         strctAnalog = fnReadDumpAnalogFile(strTrainFile1,'Resample',a2fSampleTime_PLX);
%         figure;
%         plot(afTimeRange,strctAnalog.m_afData')
%     end
%      if ~isempty(strTrainFile2)
%         afCueOnsetTS_PLX = fnTimeZoneChange(afCueOnsetTS_Kofiko,strctSync,'Kofiko','Plexon');
%         afTimeRange = -500:2500;
%         a2fSampleTime_PLX = zeros(iNumTrials, length(afTimeRange));
%         for k=1:iNumTrials
%             a2fSampleTime_PLX(k,:) = afCueOnsetTS_PLX(k) + afTimeRange/1e3;
%         end
%         strctAnalog = fnReadDumpAnalogFile(strTrainFile2,'Resample',a2fSampleTime_PLX);
%         figure;
%         plot(afTimeRange,strctAnalog.m_afData')
%     end
