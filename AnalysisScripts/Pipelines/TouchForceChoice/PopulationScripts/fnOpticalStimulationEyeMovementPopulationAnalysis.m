function fnOpticalStimulationEyeMovementPopulationAnalysis(acDataEntries)
...
strPopulationOutputfile = 'D:\Data\Doris\Electrophys\Data For Papers\FEF Optogenetics\EyeMovementDuringOpticalStimulation_Population.mat';
...
% strctEyePopulationOptical=fnOpticalStimulationEyeMovementPopulationAnalysisAux(acDataEntries, true);
% strPopulationOutputfile = 'D:\Data\Doris\Electrophys\Data For Papers\FEF Optogenetics\EyeMovementDuringOpticalStimulationPulsed_Population.mat';
% fprintf('saving population data to %s',strPopulationOutputfile);
% save(strPopulationOutputfile,'strctEyePopulationOptical','acDataEntries');
% fprintf('Done!\n');
...
%             a2fBefore = strctInterval.m_astrctTrain(iTrainIter).m_a2fDistToFixationSpotPixZeroBaseline(strctInterval.m_astrctTrain(iTrainIter).m_abTrainsWithFixationBeforeOnset,iOnsetIndex-iBeforeFixationMS:iOnsetIndex);
           ...
           strctEyePopulationOptical.m_afDepthMM(iGlobalIter) = mean(strctInterval.m_astrctTrain(iTrainIter).m_afIntervalDepthMM);
            ...
            iNumEvents = sum(strctInterval.m_astrctTrain(iTrainIter).m_abTrainsWithFixationBeforeOnset);
            iSubset = min( iNumEvents,size(strctInterval.m_astrctTrain(iTrainIter).m_a2fDistToFixationSpotPixZeroBaselineRandomEvents,1));
            ...
            fThresRand = 3*std(afMedianEyePosRandSubset(iOnsetIndex-iBeforeFixationMS:iOnsetIndex));
            ...
            if  strctEyePopulationOptical.m_afMaxDistDuringRandSubset(iGlobalIter)  > 35
                dbg = 1;
            end;
            iNumAbove = sum(afMedianEyePosRandAll(iOnsetIndex:iOnsetIndex+iAfterFixationMS ) > fThresRand);
            if iNumAbove > 0
                astrctIntervals = fnGetIntervals( afMedianEyePosRandAll(iOnsetIndex:iOnsetIndex+iAfterFixationMS ) > fThresRand);
       ...
       iNumAbove = sum(afMedianEyePosRandSubset(iOnsetIndex:iOnsetIndex+iAfterFixationMS ) > fThresRand);
            if iNumAbove > 0
                astrctIntervals = fnGetIntervals( afMedianEyePosRandSubset(iOnsetIndex:iOnsetIndex+iAfterFixationMS ) > fThresRand);
                strctEyePopulationOptical.m_afMaxIntervalLengthRandSubset(iGlobalIter) = max(cat(1,astrctIntervals.m_iLength));
            else
                strctEyePopulationOptical.m_afMaxIntervalLengthRandSubset(iGlobalIter) = 0;
            end            
