...
if ~isempty(afGrass2Onset_PLX) && ~isempty(afGrass1Onset_PLX)
...
if ~isempty(aiStimulationDuringPassiveFixation)
    % Tricky bit..
      ...
      abFixationOnScreen(aiStimulationDuringPassiveFixation(afFixationSpotSizePix>0)) = true;
       ...
       [acImageNameOnScreenDuringStim, aiDesignIndex, aiStimInDesignIndex] = fnIdentifyWhichImageWasOnScreenDuringPassiveFixation(strctParadigm, strctSync, afAllTrainOnsetsTS_Sorted);

      % Now decide based on the image name whether a fixation spot was on
      % or not...
      abFixationUsingStimuli = ismember(acImageNameOnScreenDuringStim,{'FixateGray' ,   'FixateGrayStim1'  ,  'FixateGrayStim12'   , 'FixateGrayStim2' });
      abFixationOnScreen(aiStimulationTrainsWithZeroFixationSpot) = abFixationUsingStimuli(aiStimulationTrainsWithZeroFixationSpot);
end
return;



function [acImageNameOnScreenDuringStim, aiDesignIndex, aiStimuliOnScreenDuringTrain] = fnIdentifyWhichImageWasOnScreenDuringPassiveFixation(strctParadigm, strctSync, afAllTrainOnsetsTS_Sorted)
...
afStimulusTS_Kofiko = strctParadigm.Trials.Buffer(5,:);
afStimulusTS_PLX = fnTimeZoneChange(afStimulusTS_Kofiko, strctSync,'Kofiko','Plexon');
aiStimuliOnScreenDuringTrain = fnMyInterp1(afStimulusTS_PLX, aiStimulusIndex, afAllTrainOnsetsTS_Sorted);

afDesignChangeTS_PLX = fnTimeZoneChange(strctParadigm.ImageList.TimeStamp, strctSync,'Kofiko','Plexon');
...
aiActiveDesign = fnMyInterp1(afDesignChangeTS_PLX,aiMappingToUniqueList, afAllTrainOnsetsTS_Sorted);
aiDesignIndex =aiActiveDesign;
acImageNameOnScreenDuringStim = cell(1, length(afAllTrainOnsetsTS_Sorted));
for iStimIter=1:length(afAllTrainOnsetsTS_Sorted)
    iActiveDesignForThisStim = aiMappingToUniqueList(aiActiveDesign(iStimIter));
    if isempty(acDesignNames{iActiveDesignForThisStim})
        acImageNameOnScreenDuringStim{iStimIter} = '';
        continue;
    end;
    if ~isfield(strctParadigm,'Designs') &&  strcmpi(acDesignNames{iActiveDesignForThisStim},'\\192.168.50.93\StimulusSet\PassiveFixationMicrostim\Fixate_Microstim2Ch_NoFixation.xml')
           acMediaName = {'Gray'  'GrayStim1'  'GrayStim2'  'GrayStim12'  'FixateGray'  'FixateGrayStim1'  'FixateGrayStim2'  'FixateGrayStim12'};
        ...
        if aiStimuliOnScreenDuringTrain(iStimIter) <= length(strctParadigm.Designs.Buffer{iActiveDesignForThisStim}.m_acMediaName)
            if aiStimuliOnScreenDuringTrain(iStimIter) == 0
                acImageNameOnScreenDuringStim{iStimIter} = '';
            else
