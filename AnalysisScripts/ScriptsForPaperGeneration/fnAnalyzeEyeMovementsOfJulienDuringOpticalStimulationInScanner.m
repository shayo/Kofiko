strctRuns1=load('D:\Data\Doris\Data For Publications\FEF Opto\Snapshots from free surfer\Julien\Julien_120503_RecordedRuns.mat');
strctRuns =[ strctRuns1.astrctRuns];
afTime = strctRuns(1).m_strctEyePos.m_afTime-strctRuns(1).m_strctEyePos.m_afTime(1);
   iNumRuns =length(strctRuns);

   
afGrayOnset = 2*[32,64,96,128,160];
afOpticalOnset = 2*[16,80,144,176];
afElectricalOnset = 2*[48,80,112,176];
...
afOnlyElectrical = setdiff(afElectricalOnset,afBoth);
   ...
   acCondition = {afGrayOnset,afOnlyOptical ,afBoth ,afOnlyElectrical };
N = 41600;
X = zeros(iNumRuns, N);
Y = zeros(iNumRuns, N);
for k=1:iNumRuns
