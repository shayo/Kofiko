function fnEntryPoint_OptogeneticsFEF_30_July_2013()
...
fnPrintExampleOpticalStimulationCell(strHaloSUAExample, 1,[],70,[1 0.75 0],[-1 3]);
...
fnPrintExampleOpticalStimulationCell(strArchSUAExample, 1,[],60,[0 0.69 0.31],[-0.5 1]);
...
fnPrintExampleOpticalStimulationCell(strChR2ChetaCamKIIExample2, 5,[],80,[0.54 0.09 1],[-1 3]);
...
fnPrintExampleOpticalStimulationCell(strhSyn_ChETAExample, 5,[],80,afCyan,[-1 3]);

strChR2SUAExample1 =  [g_strRootDrive,':\Data\Doris\Electrophys\Julien\Optogenetics\120229\RAW\..\Processed\Optogenetic_Analysis\Julien-120229_130405_OptogeneticsMicrostim_Channel_1_Interval19_Trigger_Grass1.mat'];
strChR2SUAExample2 =  [g_strRootDrive,':\Data\Doris\Electrophys\Bert\Optogenetics\111222\RAW\..\Processed\Optogenetic_Analysis\Bert-111222_165707_OptogeneticsMicrostim_Channel_1_Interval4_Trigger_Grass1.mat'];
...
fnPrintExampleOpticalStimulationCell(strChR2SUAExample2, 1,[1:20],80,[0 0.69 0.94],[-0.5 1]);
...
if 0
    ...
    afIncreaseRatioChR2 = fnPrintPopulationNeuralResponses(strChR2File,-500:1000,afBlue,28,450,550, 'ChR2',strChR2SUAExample2);
end

if 0
    strhSyn_ChETAFile = [g_strRootDrive,':\Data\Doris\Data For Publications\FEF Opto\hSyn_hChR2(E123A)_PopulationStats.mat'];
    strhSyn_ChETAExample= 'D:\Data\Doris\Electrophys\Anakin\Optogenetics\130309\RAW\..\Processed\Optogenetic_Analysis\Anakin-130309_152128_OptogeneticsMicrostim_Channel_1_Interval23_Trigger_Grass2.mat';
    afIncreaseRatioCheta_hSyn = fnPrintPopulationNeuralResponses(strhSyn_ChETAFile,-500:1000,afCyan,0,350,2550, 'Cheta',strhSyn_ChETAExample);
end
%4.2
...
if 0
    ...
    afIncreaseRatioCheta_CamKII = fnPrintPopulationNeuralResponses(strCamKII_ChETAFile,-500:1000,afPurple,0,350,2550,'Cheta',strChR2ChetaCamKIIExample2);
end
if 0
strArchSUAExample = [g_strRootDrive,':\Data\Doris\Electrophys\Julien\Optogenetics\120302\RAW\..\Processed\Optogenetic_Analysis\Julien-120302_145041_OptogeneticsMicrostim_Channel_1_Interval65_Trigger_Grass1.mat'];
...
fnPrintPopulationNeuralResponses(strArchFile,-500:1000,afGreen,28.5,450,550,'Arch',strArchSUAExample);
end

if 0
    strHaloSUAExample = [g_strRootDrive,':\Data\Doris\Electrophys\Julien\Optogenetics\120405\RAW\..\Processed\Optogenetic_Analysis\Julien-120405_121515_OptogeneticsMicrostim_Channel_1_Interval11_Trigger_Grass1.mat'];
    strHaloFile = [g_strRootDrive,':\Data\Doris\Data For Publications\FEF Opto\eNpHR3.0_PopulationStats.mat'];
    fnPrintPopulationNeuralResponses(strHaloFile,[-500:1500],afOrange,29,950,1100, 'Halo',strHaloSUAExample);
end
...
acSessionDataFilesBert = {...
 ...
 fnPrintChangesInSaccadeProbability(acSessionDataFilesJulien,true,'g');
...
if bPrintExampleSession
    if strColor == 'b'
        ...
        %iExampleSession =2;   fAmplitudeSelected = 35; fRangeView = 55;
        iExampleSession =1;  
        fAmplitudeSelected = 80;
        fRangeView = 40;
        fRangeView2 = 800;
    end
...
%     set(gcf,'position',[ 268   608   140    62]);
% set(gcf,'position',[ 1069         797         116          58]);
...
for iSessionIter=1:iNumSessions
    ...
    [fSaccadeDirection, afDistToSaccade,afAmplitude,abValid,afDirectionsAll,afSaccadeLatencyAllMS, fMeanBaselinePix,fStdBaselinePix] = fnGetSaccadeDirection(a2fX,a2fY,fAmplitudeThreshold,fPIX_TO_VISUAL_ANGLE);
      ...
      abSuccessfulSaccades = find(afAmplitude'>fThres & abValid) ;
    ...
    iNumOpticalSaccades = sum(abValidOptical' & afDistanceToSaccadeAngleDeg < fAngleThresholdDeg & afAmplitudeOnly2 > fAmplitudeThreshold);
    fChanceOfAbortTrial = sum(~abValidOptical)/length(abValidOptical);
    astrctSessionStat(iSessionIter).m_afSigAboveChanceOptical =fnMyBinomTest(iNumOpticalSaccades,sum(abValidOptical' ) ,fChanceOfAbortTrial,'Greater') ; 
...
cat(1,astrctSessionStat.m_afSigAboveChanceOptical)
...
fThresP=0.01;
% Tmp = cat(2,astrctSessionStat.m_aiNumTrainBoth);
% 
% [p,h]=ttest(Tmp(~abSigDir),mean(Tmp(abSigDir)))
...
for k=1:length(afNormalizedCurrent)
    ...
    plot(afNormalizedCurrent(k)+[-W W],[afMeanElectrical(k)+afSEMElectrical(k)]*ones(1,2),'k');
        ...
        if diff([afMeanBoth(k)-afSEMBoth(k),afMeanBoth(k)+afSEMBoth(k)])  > 0
    plot(afNormalizedCurrent(k)*ones(1,2),[afMeanBoth(k)-afSEMBoth(k),afMeanBoth(k)+afSEMBoth(k)],strColor);
        end
        if diff([afMeanElectrical(k)-afSEMElectrical(k),afMeanElectrical(k)+afSEMElectrical(k)]) > 0
...
abLargeAmplitude = afAmplitude > fAmplitude*fPIX_TO_VISUAL_ANGLE;
...
aiValid = find(abValid);
for j=1:length(aiValid)
    afToPlot = fPIX_TO_VISUAL_ANGLE* a2fGaze(aiValid(j),:);
    afToPlot(afToPlot >= MaxVisualAngle-2)=NaN;
    plot(-200:iMaxTime,afToPlot','color',[0.5 0.5 0.5]);
end
...
for iSpikeIter=1:length(aiSpike)
    ...
    if afX(aiSpike(iSpikeIter)) > fRangeX(1)+0.015 && ...
            afX(aiSpike(iSpikeIter)) < fRangeX(2)-0.015  && ...
            fSpikesOffset+aiTrial(iSpikeIter)*iRasterLine <= fSpikesOffset+iRasterLine*(iNumTrials-1)  
            
    plot(afX(aiSpike(iSpikeIter)),fSpikesOffset+aiTrial(iSpikeIter)*iRasterLine,'k.','MarkerSize',1);
    end
end


set(gca,'xlim', [fRangeX]);
set(gca,'ylim',[0 fSpikesOffset+iRasterLine*iNumTrials]);
if fRangeX(2)-fRangeX(1) > 2.5
    set(gca,'xtick',fRangeX(1):1:fRangeX(2))
else
set(gca,'xtick',fRangeX(1)+0.5:0.5:fRangeX(2))
end
...
set(gca,'fontsize',22);
set(fFig,'position',[    680   712   370   314]);
%%
figure(fFig+1);
...
function [afIncreaseRatio] = fnPrintPopulationNeuralResponses(strResultsFile, aiRange, afBaseColor, fIgnoreDepth, fTrainLengthLow,fTrainLengthHigh, strOpsin,strExample)
strctTmp=load(strResultsFile);
%%

% strExample='D:\Data\Doris\Electrophys\Bert\Optogenetics\120610\RAW\..\Processed\Optogenetic_Analysis\Bert-120610_111659_OptogeneticsMicrostim_Channel_1_Interval29_Trigger_Grass2.mat';

iExampleCell  = [];
for k=1:length(strctTmp.strctPopulationOpticalStim.m_acDataEntries)
    if strcmp(strExample,strctTmp.strctPopulationOpticalStim.m_acDataEntries{k}.m_strFile)
        iExampleCell = k;
    end
end
...
[A,B,C]=unique([strctTmp.strctPopulationOpticalStim.m_aiGridHoleX(abMonkeyJ & ~abIgnore & abSUA);strctTmp.strctPopulationOpticalStim.m_aiGridHoleY(abMonkeyJ & ~abIgnore & abSUA)]','rows');
aiHist = histc(C,1: size(A,1));
fprintf('Julien, units according to grid hole distribution:\n');
for j=1:size(A,1)
    fprintf('[%d, %d] : %d (%.2f)\n', A(j,1),A(j,2), aiHist(j),aiHist(j)/sum(aiHist)*1e2)
end
[A,B,C]=unique([strctTmp.strctPopulationOpticalStim.m_aiGridHoleX(~abMonkeyJ & ~abIgnore & abSUA);strctTmp.strctPopulationOpticalStim.m_aiGridHoleY(~abMonkeyJ & ~abIgnore & abSUA)]','rows');
aiHist = histc(C,1: size(A,1));
fprintf('Bert, units according to grid hole distribution:\n');
for j=1:size(A,1)
    fprintf('[%d, %d] : %d (%.2f)\n', A(j,1),A(j,2), aiHist(j),aiHist(j)/sum(aiHist)*1e2)
end
...
figure(42);
clf;
loglog(strctTmp.strctPopulationOpticalStim.m_afMeanBefore(~abIgnore & ~abSig),strctTmp.strctPopulationOpticalStim.m_afMeanDuring(~abIgnore & ~abSig),'.','color',[0.5 0.5 0.5],'markersize',iMarkerSize);
hold on;
loglog(strctTmp.strctPopulationOpticalStim.m_afMeanBefore(~abIgnore & abSig),strctTmp.strctPopulationOpticalStim.m_afMeanDuring(~abIgnore & abSig),'k.','markersize',iMarkerSize)
if ~isempty(iExampleCell)
    loglog(strctTmp.strctPopulationOpticalStim.m_afMeanBefore(iExampleCell),strctTmp.strctPopulationOpticalStim.m_afMeanDuring(iExampleCell),'r.','markersize',iMarkerSize)    
end
...
box off
set(gca,'fontsize',19);
set(42,'position',[   1013         694         318         243]);
...
switch strOpsin
    ...
    fLFPRange = 500;
    case 'Cheta'
    aiRelevantUnitsCont = find(abSig & ~abReduction & strctTmp.strctPopulationOpticalStim.m_aiNumPulsesInTrain>200 & abs(strctTmp.strctPopulationOpticalStim.m_afTrainLengthMS-2000) < 10);
    aiRange2=-1000:3000;
    fLFPRange = 800;
    case 'Halo'
...
figure(500);
clf;
bar(1:4,[fPerc200Cont,fPerc80Hz, fPerc50HzShort,fPerc50HzLong ],'facecolor',[127 127 127]/255)
...
afLasermWmm2  = [60.45, 74.77, 76.36,82.73,95.45,  159.09, 254.55, 381.82];
afIntensitiesmW = interp1(afLaserDial,afLasermW,afAmplitudes);
afIntensitiesmWmm2 = interp1(afLaserDial,afLasermWmm2,afAmplitudes);
...
figure(11);
clf;
plot(afIntensitiesmWmm2,afPercSuccessful,'k','LineWidth',2)
set(gca,'xlim',[0 400])
...
figure(12);
clf;
plot(afAmplitudes,afPercSuccessful,'k','LineWidth',1);
...
for iSessionIter=1:iNumSessions
    ...
    [fSaccadeDirection, afDistToSaccade,afAmplitude,abValid,afDirectionsAll,afSaccadeLatencyAllMS, fMeanBaselinePix,fStdBaselinePix] = fnGetSaccadeDirection(a2fX,a2fY,fAmplitudeThreshold*fPIX_TO_VISUAL_ANGLE,fPIX_TO_VISUAL_ANGLE);
    ...
    for iAmplitudeIter=1:length(afOverlappingAmplitudes)
        ...
        afAmplitudeElectricalValid = afAmplitude1(abValid1' & afDist1 < fAngleThresholdDeg & afAmplitude1> fAmplitudeThreshold*fPIX_TO_VISUAL_ANGLE);
        afAmplitudeBothValid = afAmplitude1and2(abValid1and2' &afDist1and2 < fAngleThresholdDeg & afAmplitude1and2 > fAmplitudeThreshold*fPIX_TO_VISUAL_ANGLE);
        afDirections1Valid = afDirections1(abValid1' & afDist1 < fAngleThresholdDeg & afAmplitude1 > fAmplitudeThreshold*fPIX_TO_VISUAL_ANGLE);
        afDirections1and2Valid = afDirections1and2(abValid1and2' &afDist1and2 < fAngleThresholdDeg & afAmplitude1and2 > fAmplitudeThreshold*fPIX_TO_VISUAL_ANGLE);
        ...
        afPercentSaccadesOnlyElectrical(iAmplitudeIter) = sum(abValid1' &  afDist1 < fAngleThresholdDeg & afAmplitude1 > fAmplitudeThreshold*fPIX_TO_VISUAL_ANGLE) / length(afDist1)*100;
        afPercentSaccadesElectricalandOptical(iAmplitudeIter) = sum(abValid1and2' &  afDist1and2 < fAngleThresholdDeg & afAmplitude1and2 > fAmplitudeThreshold*fPIX_TO_VISUAL_ANGLE) / length(afDist1and2)*100;
        
        % Do a binomial test .
        % Assume probability of a saccade using electrical is known
        % ...and estimate the probability of seeing the number of
        % saccades in the joint case.
        iNumSaccadesBoth = sum( afDist1and2 < fAngleThresholdDeg & afAmplitude1and2 > fAmplitudeThreshold*fPIX_TO_VISUAL_ANGLE);
