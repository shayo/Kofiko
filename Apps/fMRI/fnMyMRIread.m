function mri = fnMyMRIread(fstring,headeronly,MaxFrames)
...
if(nargin < 1 | nargin > 3)
  fprintf('mri = MRIread(fstring,headeronly,NumFrames)\n');
  return;
end
if(exist('headeronly')~=1) headeronly = 0; end
if(exist('NumFrames')~=1) NumFrames = 0; end
...
%------- nifti nii -------------------------------------   
 case {'nii','nii.gz'}
  hdr = fnMyload_nifti(fspec,headeronly,MaxFrames);
  if(isempty(hdr))
    fprintf('ERROR: loading %s as analyze\n',fspec);
    mri = [];
    return;
