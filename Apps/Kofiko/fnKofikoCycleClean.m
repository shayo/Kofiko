...
if fCycleTic - g_strctCycle.m_fSyncTimer > g_strctCycle.m_fSyncPeriodSec && ~g_strctAppConfig.m_strctStimulusServer.m_fSingleComputerMode
    ...
    if isfield(g_strctDAQParams,'m_fSyncPort') && g_bRecording
        % Send sync pulse and pick a new sync interval
        fTS = GetSecs();
        fnDAQWrapper('TTL', g_strctDAQParams.m_fSyncPort,250 * 1e-6); % 250usec TTL pulse
        if (g_strctAcquisitionServer.m_bConnected)
            fndllZeroMQ_Wrapper('Send',g_strctAcquisitionServer.m_iSocket,['KofikoSync ',sprintf('%f',fTS)]);
        end
        % Save information 
        g_strctDAQParams = fnTsSetVar(g_strctDAQParams,'RecordSync',g_strctCycle.m_fSyncPeriodSec);
        g_strctCycle.m_fSyncPeriodSec = 0.1+round(rand()*10)/10; % pick a random inter-sync interval
...
if g_strctDAQParams.m_fUseMouseClickAsEyePosition || g_strctDAQParams.m_bMouseGazeEmulator
    try
        [fMouseRawX, fMouseRawY, aiButtons] = GetMouse(g_strctDAQParams.m_fMonitorIndexForEyePosition);
    catch
        [fMouseRawX, fMouseRawY] = GetMouse(g_strctDAQParams.m_fMonitorIndexForEyePosition);
        aiButtons = [0,0,0];
    end   % aiButtons = fndllMiceHook('GetButtons',0);
    
    if g_strctDAQParams.m_bMouseGazeEmulator
        fPTB_X = fMouseRawX-g_strctPTB.m_aiScreenRect(1);
        fPTB_Y = fMouseRawY-g_strctPTB.m_aiScreenRect(2);
        
        fEyeXPix = fPTB_X / g_strctPTB.m_fScale ;
        fEyeYPix = fPTB_Y / g_strctPTB.m_fScale ;
        ...
        if fEyeXPix < g_strctStimulusServer.m_aiScreenSize(1) ||  ...
                fEyeXPix > g_strctStimulusServer.m_aiScreenSize(3) || ...
                fEyeYPix < g_strctStimulusServer.m_aiScreenSize(2) || ...
                fEyeYPix > g_strctStimulusServer.m_aiScreenSize(4)
