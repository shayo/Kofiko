function varargout = ChipKitStimGUI(varargin)
...
%
%      CHIPKITSTIMGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CHIPKITSTIMGUI.M with the given input arguments.
%
%      CHIPKITSTIMGUI('Property','Value',...) creates a new CHIPKITSTIMGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ChipKitStimGUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ChipKitStimGUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ChipKitStimGUI

% Last Modified by GUIDE v2.5 22-Oct-2012 17:10:14

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @ChipKitStimGUI_OpeningFcn, ...
    'gui_OutputFcn',  @ChipKitStimGUI_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before ChipKitStimGUI is made visible.
function ChipKitStimGUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ChipKitStimGUI (see VARARGIN)

% Choose default command line output for ChipKitStimGUI
...
% UIWAIT makes ChipKitStimGUI wait for user response (see UIRESUME)
...
function varargout = ChipKitStimGUI_OutputFcn(hObject, eventdata, handles)
...
TCP_LOAD_PRESET= 13;
mssendraw(strctParams.m_hSocket,uint8([sprintf('%02d %d %d',TCP_LOAD_PRESET,iPresetIndex-1),0]));
S=fnRecvString(strctParams.m_hSocket, 1);
fnClearMessageQueue(strctParams.m_hSocket);
[strctParams.m_astrctChannels,strctParams.m_acPresetNames]=fnReadParametersFromStimulator(strctParams.m_hSocket);
setappdata(handles.figure1,'strctParams',strctParams);
