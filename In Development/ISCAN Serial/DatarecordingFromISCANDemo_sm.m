function DatarecordingFromISCANDemo_sm(portSpec, sampleFreq, maxComponents, baudRate, specialSettings)
addpath('Z:\PublicLib\PTB\PsychBasic');
addpath('Z:\PublicLib\PTB\PsychHardware');
addpath('Z:\PublicLib\PTB\PsychOneliners');
addpath('Z:\PublicLib\PTB\PsychBasic\MatlabWindowsFilesR2007a');
%TODO make this work!!! for mac and pc
%	tested ISCAN output under windows real serial port (each entry: space + xxx.x + tab; CR/LF at end of line)
%	with real serial port it works okay
%	works with 23 bytes InBufferSize in the FTDI driver and a LatencyTimer
%	of 2 on the macbook (using the USB port closer to the firewire port
%	that is not shared with the keyboard.

%	ASCII takes 23 bytes for 3 values, maybe binary can be used to squeeze
%	the three values into 16 bytes
...
% Each component is 7 Bytes (space + xxx.x digits + tab delimiter)(actually space, xxx.x, + tab), and
...
if isempty(sampleFreq)
    ...
    sampleFreq = 120;	% switch to 60Hz in the scanner
end
...
% each:
InputBufferSize = maxReadQuantum * sampleFreq * 3600 * 6;	% actually 6 hours
...
% Start tracker:
IOPort('Write', myport, iscan_track_on_code);
...
tpkt = zeros(1, sampleFreq * 3600 * 6);	% 6 hours, actually
count = 0;
treceived = [];
...
while (1)
    ...
    A=sscanf(char(pktdata),'%f %f %f')
    fprintf('ReceiveTime: %f seconds GetSecs time: Datastring = %s. Data Values = %.2f %.2f %.2f\n', treceived, pktstring,A(1),A(2),A(3));
...
% Stop tracker:
IOPort('Write', myport, iscan_track_off_code);
