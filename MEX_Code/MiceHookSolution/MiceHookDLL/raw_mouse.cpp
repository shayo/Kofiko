...
static int read_raw_input(PRAWINPUT);
...
int read_raw_input(PRAWINPUT raw)
{
		...
		if (raw_mice[i].device_handle == raw->header.hDevice)
		{
			...
			if (include_individual_mice) {
				...
				if (raw->data.mouse.usFlags & MOUSE_MOVE_ABSOLUTE)          
					raw_mice[i].is_absolute = 1;
				else if (raw->data.mouse.usFlags & MOUSE_MOVE_RELATIVE)     
					raw_mice[i].is_absolute = 0;
				if (raw->data.mouse.usFlags & MOUSE_VIRTUAL_DESKTOP)        
					raw_mice[i].is_virtual_desktop = 1;
				else                                                        
					raw_mice[i].is_virtual_desktop = 0;
			...
			if (include_sys_mouse) {
				...
				if ((prev_x != raw_mice[RAW_SYS_MOUSE].x) || (prev_y != raw_mice[RAW_SYS_MOUSE].y) && !(raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_1_UP)) {
					raw_mice[RAW_SYS_MOUSE].buttonpressed[0] = 1;
				}
				prev_x = raw_mice[RAW_SYS_MOUSE].x;
				prev_y = raw_mice[RAW_SYS_MOUSE].y;
...
int add_to_raw_mouse_x_and_y(HANDLE in_device_handle)
{
	...
	int iDeviceID= read_raw_input((RAWINPUT*)lpb);
...
int  get_raw_mouse_name(int mousenum, char *name)
{
	ULONG nReturn = 0;

	if (raw_mice != NULL && mousenum < nraw_mouse_count) {
		// psname is actually tchar...
		// every other char is zero. not ideal....

		for (int k=0;k<raw_mice[mousenum].nSize;k++) {
			name[k] = raw_mice[mousenum].psname[2*k];
		}
		name[raw_mice[mousenum].nSize] = 0;

		return raw_mice[mousenum].nSize;
...
BOOL register_raw_mouse(HWND hwnd)
{
	...
	Rid[0].usUsage = 0x02; 
	Rid[0].dwFlags = RIDEV_INPUTSINK;//RIDEV_INPUTSINK ;// RIDEV_NOLEGACY;   // adds HID mouse and also ignores legacy mouse messages
